...
const existCheck = function (argName, arg) {
    let param = {};
    param[argName] = arg;
    return database.users.find(param);
};

const allCheck = function (user) {
    if (['name', 'id', 'mail', 'phone'].every(argName => !!user[argName] && !!user[argName].match(checkReg[argName]))) {
        return database.users.checkExist(user)
                       .then(result => !result);
    } else {
